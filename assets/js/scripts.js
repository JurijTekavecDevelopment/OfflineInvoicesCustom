...
(function ($) {
	$.fn.toJson = function (options) {
		...
		$.each(this.serializeArray(), function () {
			...
			if (name in json) {
				if (!json[name].push) json[name] = [json[name]];
				json[name].push(this.value || "");
			} else json[name] = this.value || "";
		});
...
$(document).ready(function () {
		...
		if (settings != undefined) {
			...
			if (settings.charge_vat != undefined) {
				settings.charge_vat == 1
					? $("#charge_vat").prop("checked", true)
					: $("#charge_vat").prop("checked", false);
			}

			if (settings.logo != "") {
				$("#current_img").html(
					`<img src="${stores + file_path + settings.logo}"/>`
				);
			}
	...
	$("#action_add_product").click(function (e) {
		...
		if ($("#product_name").val() == "") {
			Swal.fire("No Item Name!", "Please enter the name of the item!", "error");
			return false;
		}
	...
	$("#action_create_customer").click(function (e) {
		...
		if ($("#customerName").val() == "") {
			Swal.fire("No Customer Name!", "Please enter a customer name!", "error");
			return false;
		}
	...
	function showCompanyDetails() {
		...
		if (settings != undefined) {
			...
			if (settings.logo != "") {
				$("#logo").html(
					`<img src="${
						stores + file_path + settings.logo
					}" class="img-responsive"/>`
				);
			}
		}
		$(
			"#price_currency, #sub_currency, #invoice_sub_currency, #invoice_discount_currency, #shipping_currency, #invoice_total_currency, #vat_currency, #product_price_currency"
		).text(symbol);
		...
		invoiceDB.find({}, function (err, docs) {
			if (docs.length > 0) {
				...
				Highcharts.chart("pie", {
					...
					title: {
						text:
							group_by == "day" ? months[data_month - 1] + " " + year : year,
					},
					tooltip: {
	...
	function manageInvoices() {
		...
		invoices.forEach((invoice, index) => {
			...
			<td class="nobr"><span class="btn-group"><button onClick="$(this).openEmail(1, ${index})" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-envelope"></span> Send</button><button onClick="$(this).openPDF(${
				invoice._id
			})" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-eye-open"></span> PDF</button><button onClick="$(this).editInvoice(${index})" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-edit"> Edit</span></button><button onClick="$(this).deleteInvoice(${
				invoice._id
			})" class="btn btn-white btn-sm"><span class="glyphicon glyphicon-trash"></span> Delete</button></span></td></tr>`;
	...
	function manageQuotes() {
		...
		quotes.forEach((quote, index) => {
			...
			<td class="nobr"><span class="btn-group"><button onClick="$(this).openEmail(2, ${index})" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-envelope"></span> Send</button><button onClick="$(this).openPDF(${
				quote._id
			})" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-eye-open"></span> PDF</button><button onClick="$(this).editQuote(${index})" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-edit"> Edit</span></button><button onClick="$(this).deleteQuote(${
				quote._id
			})" class="btn btn-white btn-sm"><span class="glyphicon glyphicon-trash"></span> Delete</button></span></td></tr>`;
	...
	function manageProducts() {
		...
		products.forEach((product, index) => {
			...
			<td><span class="btn-group"><button onClick="$(this).editProduct(${index})" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-edit"></span></button><button onClick="$(this).deleteProduct(${
				product._id
			})" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></button></span></td></tr>`;
	...
	$.fn.openEmail = function (group, i) {
		...
		(async () => {
			...
			$("#smtp_status").html(
				smtp_status
					? 'SMTP Server: <span class="online">Online</span>'
					: 'SMTP Server: <span class="offline">Offline</span>'
			);
		})();
		...
		$("#email_customer").text(
			group == 1
				...
				: "Send Quotation to " + invoice.customer_name
		);
		$("#email_subject").val(
			group == 1
				? "Invoice " + invoice._id + " from " + invoice.company_name
				: "Quotation " + invoice._id + " from " + invoice.company_name
		);
		$("#email_msg").val(
			"Dear " +
	...
	$.fn.editQuote = function (i) {
		...
		$("#invoice_status").empty();
		$("#invoice_status").html(
			`<option value="pending">Pending</option><option value="accepted">Invoice</option>`
		);
		...
		const item = `<tr>
			<td>
				<div class="form-group form-group-sm  no-margin-bottom">
					...
					<input type="text" class="form-control form-group-sm item-input invoice_product" value="${
						index >= 0 ? data.invoice_product[index] : data.invoice_product
			...
			<td>
			<div class="form-group form-group-sm  no-margin-bottom">
			   <input type="text" class="form-control form-group-sm invoice_product_desc" name="invoice_product_desc[]" value="${
						index >= 0
							? data.invoice_product_desc[index]
							: data.invoice_product_desc
					}">
			</div>
		</td>
			<td class="text-right">
				<div class="form-group form-group-sm no-margin-bottom">
					<input type="text" class="form-control calculate" name="invoice_product_qty[]" value="${
						index >= 0
							? data.invoice_product_qty[index]
							: data.invoice_product_qty
					}">
				</div>
			</td>
			<td class="text-right">
				<div class="input-group input-group-sm  no-margin-bottom">
					...
					<input type="text" class="form-control calculate invoice_product_price required" value="${
						index >= 0
							? data.invoice_product_price[index]
							: data.invoice_product_price
					}" name="invoice_product_price[]" aria-describedby="sizing-addon1" placeholder="0.00">
				</div>
			</td>
			<td class="text-right">
				<div class="form-group form-group-sm  no-margin-bottom">
					<input type="text" class="form-control calculate" name="invoice_product_discount[]" value="${
						index >= 0
							? data.invoice_product_discount[index]
							: data.invoice_product_discount
			...
			<td class="text-right">
				<div class="input-group input-group-sm">
					<span class="input-group-addon">${symbol}</span>
					<input type="text" class="form-control calculate-sub" name="invoice_product_sub[]" id="invoice_product_sub" value="${
						index >= 0
							? data.invoice_product_sub[index]
							: data.invoice_product_sub
	...
	function actionCreateInvoice(data) {
		...
		if (data.invoice_id == "") {
			//let new_id = Date().getFullYear() * 10;
			let new_id = 1;
	...
	function pdfRows(data, index) {
		...
		<td class="text-left"><h3>${
			index >= 0 ? data.invoice_product_desc[index] : data.invoice_product_desc
		}</h3></td>
		<td class="unit">${
			index >= 0
				? symbol + format(data.invoice_product_price[index])
				: symbol + format(data.invoice_product_price)
		}</td>
		<td class="qty">${
			index >= 0 ? data.invoice_product_qty[index] : data.invoice_product_qty
		}</td>
		<td class="total">${
			index >= 0
				? symbol + format(data.invoice_product_sub[index])
				: symbol + format(data.invoice_product_sub)
		}</td>
		</tr>`;
		...
		const pdf_invoice = `<header>
			<div class="row">
				<div class="col-md-6">
					<a hre="#" id="logo">
						<img src="${
							settings.logo != "" ? stores + file_path + settings.logo : ""
					...
					${
						invoice.company_address != ""
							? "<div>" +
							  invoice.company_address +
							  ", " +
							  invoice.company_town +
							  "</div>"
							: ""
					}
					${
						invoice.company_email != ""
							? "<div><b>Email:</b> " + invoice.company_email + "</div>"
							: ""
					}
					${
						invoice.company_phone != ""
							? "<div><b>Phone:</b> " + invoice.company_phone + "</div>"
							: ""
					}
					${
						invoice.company_vat != ""
							? "<div><b>Vat No:</b> " + invoice.company_vat + "</div>"
							: ""
					}
				</div>
			</div>
		...
		<main>
			<div class="row contacts">
				<div class="col-md-6 invoice-to">
					<div class="text-gray-light">${
						invoice.action == "create_invoice" ? "INVOICE TO:" : "QUOTE TO:"
					}</div>
				...
				<div class="col-md-6 invoice-details pull-right">
					<h1 class="invoice-id">${
						invoice.action == "create_invoice" ? "INVOICE:" : "QUOTE:"
					} ${Date().getFullYear() + "-" + invoice._id}</h1>
					<div class="date">Date: ${invoice.invoice_date}</div>
					<div class="date">${
						invoice.action == "create_invoice" ? "Due Date:" : "Valid Until:"
					} ${invoice.invoice_due_date}</div>
				</div>
			</div>
			<table border="0" cellspacing="0" cellpadding="0">
				...
				<tfoot>
					...
					${
						invoice.invoice_discount != "0.00"
							? '<tr><td colspan="3"></td><td colspan="2">DISCOUNT</td><td>' +
							  symbol +
							  format(invoice.invoice_discount) +
							  "</td></tr>"
							: ""
					}	
					${
						charge_vat
							? '<tr><td colspan="3"></td><td colspan="2">VAT ' +
							  percentage +
							  "%</td><td>" +
							  symbol +
							  format(invoice.invoice_vat) +
							  "</td></tr>"
							: ""
					}		
					${
						invoice.invoice_shipping != ""
							? '<tr><td colspan="3"></td><td colspan="2">DELIVERY </td><td>' +
							  symbol +
							  format(invoice.invoice_shipping) +
							  "</td></tr>"
							: ""
					}	
			...
			${
				invoice.invoice_notes != ""
					? '<div>PLEASE NOTE:</div><div class="notice">' +
					  invoice.invoice_notes +
					  "</div>"
					: ""
			}
...
(async () => {
	...
	$.get(
		"http://update.offlineinvoicing.com?version=" + version,
		function (data) {
...
$("#dl").click(function () {
	...
	require("electron")
		...
		.download(
			{
				url:
					"http://update.offlineinvoicing.com/OfflineInvoicing-" +
					version +
					".msi",
			},
			function (error, info) {
	...
	if (
		shell.openItem(
			app.getPath("downloads") +
				"\\OfflineInvoicing\\OfflineInvoicing-" +
				version +
				".msi"
		)
	) {
